...
__global__ void kernel_resample2d_update_output(const int n, 
    ...
    float dx = static_cast<float>(DIM3_INDEX(input2, b, 0, y, x));
    float dy = static_cast<float>(DIM3_INDEX(input2, b, 1, y, x));

    float xf = static_cast<float>(x) + dx;
    float yf = static_cast<float>(y) + dy;
    float alpha = xf - floorf(xf); // alpha
    float beta = yf - floorf(yf); // beta

    int xL = max(min( int (floorf(xf)),    dim_w-1), 0);
    int xR = max(min( int (floorf(xf)+1), dim_w -1), 0);
    int yT = max(min( int (floorf(yf)),    dim_h-1), 0);
    int yB = max(min( int (floorf(yf)+1),  dim_h-1), 0);

    for (int fy = 0; fy < kernel_size; fy += 1) {
        for (int fx = 0; fx < kernel_size; fx += 1) {
            fval += (1.f - alpha) * (1.f - beta) * static_cast<float>(DIM3_INDEX(input1, b, c, yT + fy, xL + fx));
            fval += (alpha) * (1.f - beta) * static_cast<float>(DIM3_INDEX(input1, b, c, yT + fy, xR + fx));
            fval += (1.f - alpha) * (beta) * static_cast<float>(DIM3_INDEX(input1, b, c, yB + fy, xL + fx));
            fval += (alpha) * (beta) * static_cast<float>(DIM3_INDEX(input1, b, c, yB + fy, xR + fx));
        }
    }

    output[index] = static_cast<scalar_t>(fval);
...
__global__ void kernel_resample2d_backward_input1(
    ...
    int xL = max(min( int (floorf(xf)),    idim_w-1), 0);
    int xR = max(min( int (floorf(xf)+1), idim_w -1), 0);
    int yT = max(min( int (floorf(yf)),    idim_h-1), 0);
    int yB = max(min( int (floorf(yf)+1),  idim_h-1), 0);

    for (int fy = 0; fy < kernel_size; fy += 1) {
        for (int fx = 0; fx < kernel_size; fx += 1) {
            gpuAtomicAddNoReturn(&DIM3_INDEX(gradInput, b, c, (yT + fy), (xL + fx)), static_cast<scalar_t>((1.f - alpha)*(1.f - beta) * static_cast<float>(DIM3_INDEX(gradOutput, b, c, y, x))));
            gpuAtomicAddNoReturn(&DIM3_INDEX(gradInput, b, c, (yT + fy), (xR + fx)), static_cast<scalar_t>((alpha)*(1.f - beta) * static_cast<float>(DIM3_INDEX(gradOutput, b, c, y, x))));
            gpuAtomicAddNoReturn(&DIM3_INDEX(gradInput, b, c, (yB + fy), (xL + fx)), static_cast<scalar_t>((1.f - alpha)*(beta) * static_cast<float>(DIM3_INDEX(gradOutput, b, c, y, x))));
            gpuAtomicAddNoReturn(&DIM3_INDEX(gradInput, b, c, (yB + fy), (xR + fx)), static_cast<scalar_t>((alpha)*(beta) * static_cast<float>(DIM3_INDEX(gradOutput, b, c, y, x))));
        }
    }
...
__global__ void kernel_resample2d_backward_input2(
    ...
    if (c % 2) {
        float gamma = 1.f - (xf - floorf(xf)); // alpha
        for (int i = 0; i <= 2*kernel_rad; ++i) {
            for (int j = 0; j <= 2*kernel_rad; ++j) {
                for (int ch = 0; ch < odim_c; ++ch) {
                    output += static_cast<scalar_t>((gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yB + j), (xL + i))));
                    output -= static_cast<scalar_t>((gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yT + j), (xL + i))));
                    output += static_cast<scalar_t>((1.f-gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yB + j), (xR + i))));
                    output -= static_cast<scalar_t>((1.f-gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yT + j), (xR + i))));
                }
            }
        }
    }
    else {
        float gamma = 1.f - (yf - floorf(yf)); // alpha
        for (int i = 0; i <= 2*kernel_rad; ++i) {
            for (int j = 0; j <= 2*kernel_rad; ++j) {
                for (int ch = 0; ch < odim_c; ++ch) {
                    output += static_cast<scalar_t>((gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yT + j), (xR + i))));
                    output -= static_cast<scalar_t>((gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yT + j), (xL + i))));
                    output += static_cast<scalar_t>((1.f-gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yB + j), (xR + i))));
                    output -= static_cast<scalar_t>((1.f-gamma) * static_cast<float>(DIM3_INDEX(gradOutput, b, ch, y, x)) * static_cast<float>(DIM3_INDEX(input1, b, ch, (yB + j), (xL + i))));
                }
            }
        }
...
void resample2d_kernel_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::kHalf, at::kBFloat16, input1.scalar_type(),
        "resample_forward_kernel",
        ([&] {
...
void resample2d_kernel_backward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::kHalf, at::kBFloat16, gradInput2.scalar_type(),
        "resample_backward_input2",
        ([&] {
